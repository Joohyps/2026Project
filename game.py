...
from player  import Player, ALIVE, KNOCKED
...
class Game:
    ...
    def handle_event(self, ev):
        ...
        if self.state == MENU:
            ...
            if k == pygame.K_SPACE:  self._action(self.p1)
            if k == pygame.K_u:      self._action(self.p2)
            if k == pygame.K_ESCAPE: self.state = MENU
        elif self.state == OVER:
            if k == pygame.K_SPACE:  self._reset(); self.state = PLAY
            if k == pygame.K_ESCAPE: self._reset(); self.state = MENU
    ...
    def _auto_pickup(self):
        for p in [self.p1, self.p2]:
            if p.held or p.state == KNOCKED: continue
            for b in self.balls:
    ...
    def _hud(self):
        ...
        h = self.f_xs.render(
            "P1: WASD + Space     P2: IJKL + U     ESC: menu",
            True, (130, 130, 130))
    ...
    def _draw_menu(self):
        ...
        c1 = self.f_xs.render("Player 1   W A S D  이동   Space  줍기/던지기", True, C_SCORE1)
        self.screen.blit(c1, c1.get_rect(centerx=WIDTH // 2, centery=HEIGHT // 2 + 48))
        c2 = self.f_xs.render("Player 2   I J K L  이동     U   줍기/던지기", True, C_SCORE2)
        self.screen.blit(c2, c2.get_rect(centerx=WIDTH // 2, centery=HEIGHT // 2 + 78))
